<template>
    <div v-if="isOpen" class="fixed top-0 left-0 w-full h-full flex items-center justify-center z-50">
      ...
      <!-- Modal container -->
      <div class="relative max-w-md max-h-full w-full bg-white shadow-lg dark:bg-gray-800">
        <!-- Modal header -->
        <div class="flex justify-between items-center px-4 py-2 bg-red-500 border-b dark:bg-gray-700">
          <h3 class="text-sm font-semibold text-white">{{ title }}</h3>
          <button @click="close" class="text-gray-200 hover:text-white">
        ...
        <div class="p-4 text-left">
          ...
          <div class="flex justify-end space-x-4">
            <button 
              @click="close" 
              class="px-4 py-2 text-sm text-gray-600 bg-gray-200 focus:outline-none hover:underline"
            >
              Cancel
            </button>
            <button 
  ...
  const confirmDelete = () => {
    emit('confirm', props.patientId); // Emit patient ID for deletion
  };
